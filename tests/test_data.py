"""Tests for the data layer - mostly sanity checks that the synthetic generator
produces plausible, well-shaped data and that the haven label behaves itself.
"""
...
def test_schema_has_financials_and_activities():
    # The generated table should have exactly the columns we expect - all the
    # activity types and the core financials - so nothing downstream is surprised.
...
def test_no_negative_revenue_or_employees():
    # Basic realism check: you can't have negative revenue or fewer than one
    # employee, so the generator shouldn't ever emit those.
...
def test_reproducible_with_seed():
    # Same seed in, same data out - if this breaks, results stop being repeatable.
...
def test_supplementary_adds_haven_flag():
    # The supplementary step is what attaches the haven label, and it must be a
    # clean 0/1 flag - nothing fuzzy.
...
def test_havens_have_higher_holding_share():
    # The synthetic data should at least reflect the real-world pattern we're
    # trying to detect: havens tend to host more pure holding activity. If even
    # the toy data didn't show this, the whole setup would be questionable.
